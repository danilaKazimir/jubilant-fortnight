...
class TestOperations:
    ...
    def test_create_operations(self, operations_client: OperationsClient):
        response = operations_client.create_operation_api()

        assert_status_code(response.status_code, HTTPStatus.OK)
        validate_json_schema(response.json(), OperationSchema.model_json_schema())

    @allure.title("Get operation")
    def test_get_operation(
            self,
            operations_client: OperationsClient,
            function_operation: OperationsSchema
    ):
